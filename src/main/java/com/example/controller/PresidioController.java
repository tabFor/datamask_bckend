...
public class PresidioController {
     ...
     * 测试Presidio连接状态
     */
    @Operation(summary = "检查Presidio服务状态", description = "检查Presidio分析器和匿名化器服务的连接状态，并返回简单的测试结果。用于监控和确认敏感数据处理服务是否正常运行。")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "成功检查服务状态")
    })
    @GetMapping("/status")
     ...
     * 测试Presidio文本分析
     */
    @Operation(summary = "分析文本中的敏感信息", description = "使用Presidio分析器识别文本中的敏感信息，如个人身份信息、信用卡号、电话号码、地址等，返回识别结果及其位置和类型。")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "成功分析文本"),
            @ApiResponse(responseCode = "400", description = "请求参数错误，如文本为空")
    })
    @PostMapping("/analyze")
    public ResponseEntity<Map<String, Object>> analyzeText(
            @Parameter(description = "包含待分析文本的请求体") @RequestBody Map<String, String> request) {
     ...
     * 测试Presidio文本脱敏
     */
    @Operation(summary = "脱敏文本中的敏感信息", description = "使用Presidio自动识别并脱敏文本中的敏感信息，适用于需要保护隐私数据但保留文本结构的场景。支持多种脱敏策略，如替换、掩码、删除等。")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "成功脱敏文本"),
            @ApiResponse(responseCode = "400", description = "请求参数错误，如文本为空")
    })
    @PostMapping("/anonymize")
    public ResponseEntity<Map<String, Object>> anonymizeText(
            @Parameter(description = "包含待脱敏文本的请求体") @RequestBody Map<String, String> request) {
        String text = request.getOrDefault("text", "");
     ...
     * 使用Presidio自动识别敏感列
     */
    @Operation(summary = "自动识别数据库表中的敏感列", description = "使用Presidio自动分析数据库表内容，识别可能包含敏感信息的列，如身份证号、手机号码、地址等。支持多种数据库类型，可用于数据治理和合规检查。")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "成功识别敏感列"),
            @ApiResponse(responseCode = "400", description = "请求参数错误，如表名为空"),
            @ApiResponse(responseCode = "500", description = "服务器内部错误，如数据库连接失败")
    })
    @PostMapping("/detect-sensitive-columns")
    public ResponseEntity<Map<String, Object>> detectSensitiveColumns(
            @Parameter(description = "数据库表分析请求，包含表名和数据库连接信息") @RequestBody Map<String, String> request) {
        ...
        try {
            ...
            if (dbType != null && host != null && port != null &&
                ...
                try {
                    ...
                    sensitiveColumns = sensitiveDataDetector.detectSensitiveColumnsWithPresidioAndConnection(connection,
                            tableName);
                } finally {
                    // 关闭连接
     ...
     * 获取数据库连接
     */
    private Connection getDatabaseConnection(String dbType, String host, String port, String dbName, String username,
