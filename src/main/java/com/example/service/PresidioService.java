...
public class PresidioService {
    ...
    public List<Map<String, Object>> analyzeText(String text) {
        try {
            ...
            // 添加自定义敏感信息识别器
            List<Map<String, Object>> adHocRecognizers = new ArrayList<>();
            
            // 1. 个人身份识别
            addPersonalIdentifiers(adHocRecognizers);
            
            // 2. 联系信息识别
            addContactInformation(adHocRecognizers);
            
            // 3. 金融信息识别
            addFinancialInformation(adHocRecognizers);
            
            // 4. 医疗信息识别
            addHealthInformation(adHocRecognizers);
            
            // 5. 位置信息识别
            addLocationInformation(adHocRecognizers);
            
            // 6. 账号和密码识别
            addAccountInformation(adHocRecognizers);
            
            // 7. 职业和教育信息
            addProfessionalInformation(adHocRecognizers);
            
            // 将自定义识别器添加到请求中
            requestBody.put("ad_hoc_recognizers", adHocRecognizers);
    ...
    private void addPersonalIdentifiers(List<Map<String, Object>> recognizers) {
        ...
        chinaIdPattern.put("regex", "\\b[1-9]\\d{5}(?:19|20)\\d{2}(?:0[1-9]|1[0-2])(?:0[1-9]|[12]\\d|3[01])\\d{3}(?:\\d|X)\\b");
    ...
    private void addContactInformation(List<Map<String, Object>> recognizers) {
        ...
        ipv4Pattern.put("regex", "\\b(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\\b");
    ...
    private void addLocationInformation(List<Map<String, Object>> recognizers) {
        ...
        coordinatesPattern.put("regex", "\\b[-+]?([1-8]?\\d(\\.\\d+)?|90(\\.0+)?),\\s*[-+]?(180(\\.0+)?|((1[0-7]\\d)|([1-9]?\\d))(\\.\\d+)?)\\b");
        ...
        coordinatesRecognizer.put("context", Arrays.asList("coordinates", "latitude", "longitude", "gps", "position", "经纬度"));
    ...
    private void addProfessionalInformation(List<Map<String, Object>> recognizers) {
        ...
        studentIdRecognizer.put("context", Arrays.asList("student", "ID", "college", "university", "school", "学号", "学生"));
